...
async function calculateBasicScores(url, browser) {
  try {
    ...
    const seo =
      (metrics.hasTitle ? 40 : 0) + (metrics.hasDescription ? 40 : 0) + (metrics.hasH1 ? 20 : 0);
...
async function runAxeCore(page) {
  ...
  try {
    ...
    return results.violations.map((violation) => ({
      ...
      target:
        violation.nodes.length > 0 && violation.nodes[0].target ? violation.nodes[0].target : null,
    }));
  } catch (error) {
...
async function getAllImages($, page = null) {
  ...
  $('svg').each((_, svg) => {
    ...
    images.push({
      ...
      location:
        $svg.closest('header').length > 0
          ? 'header'
          : $svg.closest('nav').length > 0
            ? 'nav'
            : $svg.closest('footer').length > 0
              ? 'footer'
              : 'content',
...
async function analyzeImages($) {
  ...
  $('svg').each((_, svg) => {
    ...
    const hasAccessibleName =
      ariaLabel.length > 0 || ariaLabelledby.length > 0 || titleText.length > 0;
    ...
    if (!isDecorative && !hasAccessibleName) {
      ...
      issues.push({
        ...
        details: {
          ...
          id,
          suggestion:
            '装飾目的なら <svg role="presentation" ...> または aria-hidden="true" を追加。意味があるSVGなら aria-label や <title> を追加。',
        },
      });
    }
  });
...
async function countPages(startUrl, auth = null) {
  ...
  try {
    ...
    const finalStartUrl =
      normalizedStartUrl.endsWith('/') && normalizedStartUrl !== startUrlObj.origin + '/'
