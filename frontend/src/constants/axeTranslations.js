...
export const axeTranslations = {
    ...
    'button-name': {
        help: 'ボタンには識別可能な名前が必要です',
        description: 'ボタン要素にアクセシブルな名前がないと、ユーザーがボタンの機能を理解できません。',
        fixHint: 'ボタン内にテキストを追加するか、aria-label属性でボタンの機能を説明してください。画像ボタンの場合はalt属性を使用してください。'
    },
    'link-in-text-block': {
    ...
    // リスト関連
    'listitem': {
        help: 'リスト項目は適切なリストコンテナ内に配置する必要があります',
        description: 'li要素は ul、ol、menu 要素の直接の子要素として配置する必要があります。適切でないリスト構造は、スクリーンリーダーユーザーがコンテンツの構造を理解することを困難にします。',
        fixHint: '孤立したli要素を適切なul（順序なしリスト）またはol（順序ありリスト）要素で囲んでください。装飾目的の場合はlist-style: noneを使用し、セマンティックな構造を保持してください。'
    },
    'list': {
        help: 'リストには適切なリスト項目が含まれている必要があります',
        description: 'ul、ol要素には直接の子要素としてli要素のみを含める必要があります。',
        fixHint: 'リスト要素の直接の子要素はli要素のみにしてください。その他のコンテンツはli要素内に配置してください。'
    },
